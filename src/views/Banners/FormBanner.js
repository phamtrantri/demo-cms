...
import { articleBanner, category, language, auth } from 'reducers';
...
function FormBanner(props) {
  ...
  const defaultType = CATEGORY_TYPE.BANNER;
  ...
  return (
    <Formik
      ...
      validationSchema={yup.object().shape({
        ...
        thumbnail_image: yup.string().required()
      })}
    >
      {({ values: { thumbnail_image, language }, setFieldValue }) => {
        return (
          <Form>
            <Card className={classes.cardMB}>
              <CardHeader color="primary">
                <h4 className={classes.cardTitleWhite}>
                  {data._id ? t('updateBanner') : t('addBanner')}
                </h4>
              </CardHeader>
              <CardBody>
                <Field
                ...
                <Field
                  labelText={
                    t('thumbnailImage') +
                    ' * (kích thước đề nghị tối đa: 1440 x 400)'
                  }
                  id="thumbnail_image"
...
const mapStateToProps = createStructuredSelector({
  ...
  data: articleBanner.selectors.item,

  user: auth.selectors.user,

  isLoading: createSelector(
    articleBanner.selectors.isLoadingItem,
    articleBanner.selectors.isInserting,
...
const mapDispatchToProps = {
  ...
  getArticle: articleBanner.actions.articleBannerGetAjax,

  addArticle: articleBanner.actions.articleBannerInsertAjax,

  updateArticle: articleBanner.actions.articleBannerUpdateAjax,

  cleanArticle: articleBanner.actions.articleBannerGetClean
};
...
export default compose(...enhancers)(FormBanner);
