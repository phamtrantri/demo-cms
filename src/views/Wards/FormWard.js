...
import { ward, language } from 'reducers';
...
function FormService(props) {
  ...
  useEffect(() => {
    if (id) {
      props.getWard({ api: api.getWard, id });
    }
    return () => {
      props.cleanWard();
    };
  ...
  const handleSubmit = (v, bag) => {
    ...
    if (id) {
      props.updateWard({ api: api.updateWard, params });
    } else {
      props.addWard({ api: api.addWard, params, onSuccess });
    }
  };
...
const mapStateToProps = createStructuredSelector({
  ...
  data: ward.selectors.item,

  isLoading: createSelector(
    ward.selectors.isUpdating,
    ward.selectors.isInserting,
    ward.selectors.isLoadingItem,
...
const mapDispatchToProps = {
  getWard: ward.actions.wardGetAjax,

  addWard: ward.actions.wardInsertAjax,

  updateWard: ward.actions.wardUpdateAjax,

  cleanWard: ward.actions.wardGetClean,
