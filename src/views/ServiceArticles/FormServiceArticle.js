...
import { article, category, language, service, auth } from 'reducers';
...
function FormServiceArticle(props) {
  const classes = useStyles();

  const { t, categories, languages, services, data, user, isLoading } = props;

  const { id } = queryString.parse(props.location.search);

  const defaultType = CATEGORY_TYPE.DICH_VU;
  ...
  const servicesData = services.filter(i => i.status === 1);
...
export default compose(...enhancers)(FormServiceArticle);
