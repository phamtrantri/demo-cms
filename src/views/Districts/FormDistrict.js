...
import { district, language } from 'reducers';
...
function FormService(props) {
  ...
  useEffect(() => {
    if (id) {
      props.getDistrict({ api: api.getDistrict, id });
    }
    return () => {
      props.cleanDistrict();
    };
  ...
  const handleSubmit = (v, bag) => {
    ...
    if (id) {
      props.updateDistrict({ api: api.updateDistrict, params });
    } else {
      props.addDistrict({ api: api.addDistrict, params, onSuccess });
    }
  };
...
const mapStateToProps = createStructuredSelector({
  ...
  data: district.selectors.item,

  isLoading: createSelector(
    district.selectors.isUpdating,
    district.selectors.isInserting,
    district.selectors.isLoadingItem,
...
const mapDispatchToProps = {
  getDistrict: district.actions.districtGetAjax,

  addDistrict: district.actions.districtInsertAjax,

  updateDistrict: district.actions.districtUpdateAjax,

  cleanDistrict: district.actions.districtGetClean,
