...
import { article, category, language, department, auth } from 'reducers';
...
function FormDepartmentArticle(props) {
  ...
  const defaultType = CATEGORY_TYPE.CHUYEN_KHOA;
  ...
  const departmentsData = departments.filter(i => i.status === 1);
  ...
  const initData =
    id && !isEmpty(data)
      ? {
          ...
          department: data.department ? data.department._id : null
        }
      : {
          language: 'vi',
          title: '',
  ...
  return (
    <Formik
      ...
      // onReset={(v, bag) => {
      //   bag.resetForm reset the form => onReset triggered => call bag.resetForm => onReset trigger => infinite loop
      //   bag.resetForm(initData);
      //   editorRef.current.resetContent();
      // }}
            ...
            <Card className={classes.cardMB}>
              <CardHeader color="primary">
                <h4 className={classes.cardTitleWhite}>
                  {data._id
                    ? t('updateDepartmentArticle')
                    : t('addDepartmentArticle')}
                </h4>
              </CardHeader>
              <CardBody>
                <Field
                ...
                <Field
                  ...
                  options={departmentsData
                    ...
                    .map(i => ({
                      value: i._id,
                      label: i.contents.find(i => i.language === language).title
                    }))}
                />
...
export default compose(...enhancers)(FormDepartmentArticle);
