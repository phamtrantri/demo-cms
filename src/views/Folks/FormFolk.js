...
import { folk, language } from 'reducers';

import CommonTab from './components/CommonTab';
import LanguageTab from './components/LanguageTab';
import withLoading from 'utils/loading';

function FormFolk(props) {
  const {
    t,
    languages,
    data,
  ...
  useEffect(() => {
    if (id) {
      props.getFolk({ api: api.getFolk, id });
    }
    return () => {
      props.cleanFolk();
    };
  ...
  const handleSubmit = (v, bag) => {
    ...
    if (id) {
      props.updateFolk({ api: api.updateFolk, params });
    } else {
      props.addFolk({ api: api.addFolk, params, onSuccess });
    }
  };
...
const mapStateToProps = createStructuredSelector({
  ...
  data: folk.selectors.item,

  isLoading: createSelector(
    folk.selectors.isUpdating,
    folk.selectors.isInserting,
    folk.selectors.isLoadingItem,
...
const mapDispatchToProps = {
  getFolk: folk.actions.folkGetAjax,

  addFolk: folk.actions.folkInsertAjax,

  updateFolk: folk.actions.folkUpdateAjax,

  cleanFolk: folk.actions.folkGetClean,
...
export default compose(...enhancers)(FormFolk);
